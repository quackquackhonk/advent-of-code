...
use crate::almanac::{Almanac, AlmanacRange, ConversionMap};
...
pub fn parse_almanac_inner(input: &str) -> IResult<&str, Almanac> {
...
fn parse_almanac_range(input: &str) -> IResult<&str, AlmanacRange> {
    ...
    Ok((input, AlmanacRange::new(destination, source, range)))
}
